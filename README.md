...
This repository contains a Jupyter Notebook in Python that takes a system matrix A, a desired number of terms, and integration bounds as inputs to compute the state-transition matrix via the Peano-Baker Series. The program will allow LTI and LTV inputs, print partial sums to show convergence as terms are added, and plot each element of $\phi$ over time to show behavior. The instructions and documentation is presented in the form of comments throughout the code so that the user is able to follow along with each section. 
